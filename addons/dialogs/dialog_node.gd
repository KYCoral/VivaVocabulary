...
@export var extenal_file: String = "*.json"  # Replace with actual path
		...
		{'question', ..}:
			...
			for o in event['options']:
				...
				if event.has('variable'):
					button.button_pressed.connect("pressed", self, "_on_option_selected", [button, event['variable'], o])
				else:
					# Checking for checkpoints
					if o['value'] == '0':
						button.button_pressed.connect("pressed", self, "change_position", [button, int(event['checkpoint'])])
					else:
						# Continue
						button.button_pressed.connect("pressed", self, "change_position", [button, 0])
